export async function onRequest(context) {
  ...
  try {
    ...
    const [movieResponse, tvResponse] = await Promise.all([
      fetch(`https://api.themoviedb.org/3/movie/popular?api_key=${TMDB_API_KEY}&language=zh-CN`, { headers }),
      fetch(`https://api.themoviedb.org/3/tv/popular?api_key=${TMDB_API_KEY}&language=zh-CN`, { headers })
    ]);
    ...
    return new Response(JSON.stringify({ error: '获取热门影视失败' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
