export async function onRequest(context) {
  ...
  try {
    ...
    const [movieResponse, tvResponse] = await Promise.all([
      fetch(`https://api.themoviedb.org/3/search/movie?api_key=${TMDB_API_KEY}&language=zh-CN&query=${encodeURIComponent(keyword)}`, { headers }),
      fetch(`https://api.themoviedb.org/3/search/tv?api_key=${TMDB_API_KEY}&language=zh-CN&query=${encodeURIComponent(keyword)}`, { headers })
    ]);

    const [movieData, tvData] = await Promise.all([movieResponse.json(), tvResponse.json()]);
    
    const movies = movieData.results.slice(0, 6).map(m => transformMovie(m, 'movie'));
    const tvShows = tvData.results.slice(0, 6).map(t => transformMovie(t, 'tv'));
